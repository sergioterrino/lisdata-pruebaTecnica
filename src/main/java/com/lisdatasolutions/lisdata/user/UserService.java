...
public class UserService {
  ...
  public List<UserWithInactiveVehicles> getActiveUsersWithInactiveVehicles1() {
    ...
    for (User user : activeUsers) {
      ...
      // el stream() convierte la lista de vehiculos en un stream para poder
      // filtrarlos
      // el filter() filtra los vehiculos inactivos
      // el collect() convierte el stream en una lista.
      List<Vehicle> inactiveVehicles = user.getVehicles().stream()
  ...
  // it's the same but with streams
  public List<UserWithInactiveVehicles> getActiveUsersWithInactiveVehicles2() {
    List<User> users = userRepository.findByActiveTrue();
    return users.stream()
        .map(user -> {
          List<Vehicle> vehicles = user.getVehicles().stream()
              .filter(v -> !v.isActive())
              .collect(Collectors.toList());
          return new UserWithInactiveVehicles(user, vehicles);
        }).collect(Collectors.toList());
  }
